...
// Copyright (c) 2014 The Sambacoin developers
...
#if defined(NDEBUG)
# error "Sambacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x378d9b3ea1d242086930f4535b96991cb0e12fa00c396a66cf5f00a010d6f8e8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sambacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sambacoin Signed Message:\n";
...
{
    // Sambacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nDiffChangeTarget = 110000; // Patch effective @ block 110000
static const int64 nDiffChangeTargetTestnet = 10; // Patch effective @ block 10
static const int64 patchBlockRewardDuration = 6720; // 6720 blocks main net change


int64 GetSMBSubsidy(int nHeight) {
   // thanks to RealSolid & WDC and gmaz for helping out with this code
   int64_t qSubsidy = 400*COIN;
   int blocks = nHeight - nDiffChangeTarget;
   int weeks = (blocks / patchBlockRewardDuration)+1;
   //decrease reward by 0.46% every week
   for(int i = 0; i < weeks; i++) qSubsidy -= (qSubsidy/160);
   return qSubsidy;
}
...
int64 GetBlockValue(int nHeight, int64_t nFees)
{
    ...
    if(nHeight < nDiffChangeTarget) {
        if(nHeight <= 150)
        {
	    nSubsidy = 250000 * COIN; // First huge block for miners! Good luck!
        }
        if(nHeight > 150 && nHeight <= 300)
        {
	    nSubsidy = 50000 * COIN; // second huge block for first miners! Good luck!
        }
        else if(nHeight > 6850 && nHeight < 10580)
        {
	    nSubsidy = 4000 * COIN; // Carnival 2014 in Brazil, great Samba time!!!
        }
	// This will never match because Digishield and hardfork code above, just for historical reasons
        // Subsidy is cut in half every 1051800 blocks, which will occur approximately every 3 years
        nSubsidy >>= (nHeight / 1051800); // Sambacoin: 1051k blocks in ~3 years

    } else {
        nSubsidy = GetSMBSubsidy(nHeight);
    }

    ////make sure the reward is at least 1 SMB
    if(nSubsidy < COIN) {
        nSubsidy = COIN;
    }
...
static const int64 nTargetTimespanRe = 1*90; // 90 Seconds
static const int64 nTargetSpacingRe = 1*90; // 90 seconds
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
    ...
    {
        if ( fTestNet ){
            if(nBestHeight+1<nDiffChangeTargetTestnet){
...
//if v2.0 changes are in effect for block num, alter retarget values
   if(fNewDifficultyProtocol) {
    ...
    // Sambacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sambacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Sambacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x378d9b3ea1d242086930f4535b96991cb0e12fa00c396a66cf5f00a010d6f8e8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Miss you so much VIL. I hope you are ok";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x7ec705b04d69550ae796b05043e5ce6efd2b349c7ecfd79f0f776a73029b79b9"));
//
//
//
        block.print();
...
unsigned char pchMessageStart[4] = { 0xeb, 0xa0, 0x06, 0x2b }; // Sambacoin: changed only first number from litecoin value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SambacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SambacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SambacoinMiner(CWallet *pwallet)
{
    printf("SambacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sambacoin-miner");
    ...
    try { loop {
        ...
        printf("Running SambacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SambacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SambacoinMiner, pwallet));
}
